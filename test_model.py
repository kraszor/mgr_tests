...
class BinaryClassifier(nn.Module):
    def __init__(self, input_dim=4112, hidden_dims=[512, 256, 128], dropout=0.3):
...
def train_model_tune(config, data=None, checkpoint_dir=None):
    ...
    model = BinaryClassifier(
        input_dim=4112, hidden_dims=config["hidden_dims"], dropout=config["dropout"]
    ).to(device)
    ...
    for epoch in range(config["epochs"]):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "best_model_ray_tune.pth")
...
def tune_hyperparameters(train_loader, val_loader, num_samples=15, max_num_epochs=10):
    ...
    result = tune.run(
        ...
        name="raytune_binary_classifier",
        resume="AUTO",
    )
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("best_model.pth", map_location="gpu"))
    model.eval()
